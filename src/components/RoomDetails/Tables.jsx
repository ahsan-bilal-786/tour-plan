    ...
    <div className="flex-1">
      {map(data, ({ Icon, ...row }, key) => (
        <div className={`flex flex-row min-h-[54px] items-center ${key%2 === 1 ? "bg-tp-gray-400" : ""} `} key={key}>
          <div className="flex-1 font-medium text-13 pl-4 uppercase">
            <Icon />
            {row.label}
          </div>
          <div className="flex-1  font-bold text-lg">{row.value}</div>
        </div>
      ))}
    </div>
  );
...
const Tables = () => {
  return (
    <Container
      bg="bg-white"
      className="pt-[98px] pb-[102px]"
    >
      <div className="flex xs:flex-col md:flex-row gap-10 justify-between mb-38">
        <TableComponents />
        <TableComponents />
      </div>
      <p className="font-normal text-lg leading-7">To request this room, provide Disney with the following text:</p>
      <p className="font-normal text-lg leading-7 text-tp-purple">
